...
model, distribution = compile_HybridTanH_model(num_blocks, 
                                           ...
                                           num_cond_inputs=2, device=device)
...
for epoch in range(1, params["epochs"]+1):
    ...
    for batch_idx, (mu, logvar, e, theta, e_sum) in enumerate(train_loader):
        ...
        energy_sum = (energy_sum/1e4).float()
        
        # scale conditional labels
        E_true = (E_true/100).float()
        theta_true = (theta_true/np.radians(90.0)).float()
        ...
        context = torch.cat((E_true, theta_true), 1)            
    ...
    torch.save({
        ...
        latent_file_path + 'Flow_Latent_Data/FLow_test_{}.pth'.format(epoch)
    )
