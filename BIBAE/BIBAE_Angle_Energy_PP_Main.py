...
from data_utils.data_loader import MaxwellBatchLoader, MaxwellBatchLoader_Angular
...
#############################################################################
 ...
 ############################### CALIBRATED DATA TRAINING #####################################################       
        ...
        model = models.BiBAE_F_3D_BatchStat_Energy_Angle_30x30x60(params, device=device, 
                                                         z_rand=(params["latent"]-params["latentSmL"]),
                                                         z_enc=params["latentSmL"]).to(device)   
        model_P = models.PostProcess_Size1Conv_Energy_Angle_30x30x60_withA_cond_6conv_no_norm_ESum(bias=True, out_funct='none').to(device)
        train = functions.train_BiBAE_F_linear_EandA_Reset
...
################### CALIB PP, ESum only in PP model ##########################################################################
        ...
        model = models.BiBAE_F_3D_BatchStat_Energy_Angle_30x30x60(params, device=device, 
                                                         ...
                                                         z_enc=params["latentSmL"]).to(device)   
        model_P = models.PostProcess_Size1Conv_Energy_Angle_30x30x60_withA_cond_6conv_no_norm_ESum(bias=True, out_funct='none').to(device)
        train = functions.train_BiBAE_F_linear_EandA_Reset_withA_cond_PP_ESum
        test = functions.test_BiBAE_F_linear
        LATENT_DIM = params["latent"]+1
        tf = tf_Photons_thresh_calib      
    ...
    elif params['DataLoaderType'] == 'FastMaxwell':
        train_loader = MaxwellBatchLoader(file_path=params["input_path"], train_size=params["train_size"], 
                                          batch_size=params["batch_size"], shuffle=params["shuffle"], 
                                          transform=tf)
        test_loader =  MaxwellBatchLoader(file_path=params["input_path"], train_size=int(0.05*params["train_size"]), 
...
#############################################################################
# ECAL Photon energy and angle conditioning with Post Processing
#############################################################################              
