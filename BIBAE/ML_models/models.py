...
class Discriminator_F_BatchStat_Energy_Angle_30x30x60(nn.Module):
    def __init__(self, isize=30, nc=2, ndf=64):
        super(Discriminator_F_BatchStat_Energy_Angle_30x30x60, self).__init__()    
        self.ndf = ndf
        self.isize = isize
        self.nc = nc
        self.size_embed = 16
        ...
        self.diff_lin = torch.nn.Linear(30*30*60, 64)
        self.diff_log = torch.nn.Linear(30*30*60, 64) 
        
        
        self.conv1a = torch.nn.Conv3d(1, ndf, kernel_size=(2,2,3), stride=(1,1,2), padding=0, bias=False)
        self.ln1a = torch.nn.LayerNorm([29,29,29])
        ...
        self.conv2a = torch.nn.Conv3d(1, ndf, kernel_size=(2,2,3), stride=(1,1,2), padding=0, bias=False)
        self.ln2a = torch.nn.LayerNorm([29,29,29])
        ...
        self.fc1 = torch.nn.Linear(64*8, 256)
        self.fc2 = torch.nn.Linear(256,  256)
        self.fc3 = torch.nn.Linear(256, 1)


    def forward(self, img, E_true, theta_true):
        ...
        xa = torch.cat((xa1, xa2, xa3, xa4, xa5, xa6, xa7, xa8), 1)
...
class BiBAE_F_3D_BatchStat_Energy_Angle_30x30x60(nn.Module):
    """
    VAE component, with direct energy conditioning (giving true energy/angle to both en- and de-coder)
    designed for 30x30x60 images
    """
    def __init__(self, args, device, nc=1, ngf=8, z_rand=500, z_enc=12):
        super(BiBAE_F_3D_BatchStat_Energy_Angle_30x30x60, self).__init__()    
        self.ngf = ngf
        self.nc = nc
        self.z_rand = z_rand
        self.z_enc = z_enc
        ...
        self.enconv1 = nn.Conv3d(in_channels=1, out_channels=ngf, kernel_size=(2,2,3), stride=(1,1,2),
                               padding=(0,0,0), bias=False, padding_mode='zeros')
        self.bnen1   = torch.nn.LayerNorm([29,29,29])
        ...
        self.fc1 = nn.Linear(6*6*6*ngf*4+1+1, ngf*100, bias=True)   # add +1+1 for both energy and angular labels
    ...
    def encode(self, x, E_true, theta_true):
        ...
        x = torch.cat( (x.view(-1, x.size(1)*x.size(2)*x.size(3)*x.size(4)), E_true, theta_true), 1)
    ...
    def decode(self, z):
        ...
        x = x.view(-1,self.ngf,7,7,15)        

        x = F.leaky_relu(self.bnde1(self.deconv1(x, output_size=[x.size(0), 1, 15, 15, 30])), 0.2, inplace=True) #
        x = F.leaky_relu(self.bnde2(self.deconv2(x, output_size=[x.size(0), 1, 30, 30, 60])), 0.2, inplace=True) #
    ...
    def forward(self, x, E_true, theta_true, z=None, mode='full'):
        if mode == 'encode':
            mu, logvar = self.encode(x, E_true, theta_true)
            z = self.reparameterize(mu, logvar)
            return mu, logvar, z 
        elif mode == 'decode':
            return self.decode(torch.cat((z,E_true, theta_true), 1)) #, E_true 
        elif mode == 'full':
            mu, logvar = self.encode(x,E_true,theta_true)
            z = self.reparameterize(mu, logvar)
            return self.decode(torch.cat((z,E_true,theta_true), 1)), mu, logvar, z
        elif mode == 'half':
            mu, logvar = self.encode(x,E_true,theta_true)
            z = self.reparameterize(mu, logvar)
            return self.decode(torch.cat((z,E_true,theta_true), 1))       

######## For Post Processing when PP model is conditioned on the energy sum ########            
class PostProcess_Size1Conv_Energy_Angle_30x30x60_withA_cond_6conv_no_norm_ESum(nn.Module):
    def __init__(self, isize=60, isize2=30, nc=2, ndf=128, bias=False, out_funct='relu'):
        super(PostProcess_Size1Conv_Energy_Angle_30x30x60_withA_cond_6conv_no_norm_ESum, self).__init__()    
        self.ndf = ndf
        self.isize = isize
        self.isize2 = isize2
        self.nc = nc
        ...
        self.fcec1 = torch.nn.Linear(4, int(ndf/2), bias=True)
    ...
    def forward(self, img, E_True=0, theta_True=0, ESum=0):
        img = img.view(-1, 1, self.isize2, self.isize2, self.isize) 
                
        econd = torch.cat((torch.sum(img.view(-1, self.isize2*self.isize2*self.isize), 1).view(-1, 1), E_True, theta_True, ESum), 1)
