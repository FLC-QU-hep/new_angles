...
def calc_gradient_penalty_EandA_F_DIRENR(netD, real_data, fake_data, device, batchsize, E_true, theta_true, LAMBDA = 10.0):
    ...
    disc_interpolates = netD(interpolates, E_true=E_true, theta_true=theta_true)
...
def test_BiBAE_F_linear(model, netD, netD_L, epoch, test_loader, device, args, optimizer, L_Losses, L_D, L_D_L):    
    ...
    test_loss_sample = 0
    test_loss_recon = 0
    test_weighted_sample = torch.zeros(30)
    test_weighted_recon = torch.zeros(30)
    test_unwe_sample = torch.zeros(30)
    test_unwe_recon = torch.zeros(30)
    test_lossD_sample = 0
    test_lossD_recon = 0
    ...
    with torch.no_grad():
        for i, (data, energy) in enumerate(test_loader):
            ...
            sum_true = torch.sum((data.view(-1, input_size_1*input_size_2*input_size_3)), dim=1)*1e-4 #MeV to GeV
            sum_recon = torch.sum((recon_batch.view(-1, input_size_1*input_size_2*input_size_3)), dim=1)*1e-4 #MeV to GeV
            sum_sample = torch.sum((sample_batch.view(-1, input_size_1*input_size_2*input_size_3)), dim=1)*1e-4 #MeV to GeV
            ...
            loss_sample,weighted_sample,unwe_sample,name_sample = loss_function_VAE_ENR(recon_x=sample_batch, x=data, 
                                                   mu=torch.zeros_like(mu), logvar=torch.ones_like(logvar),
            ...
            fake_data_recon = torch.cat((data.view(-1,1,input_size_1,input_size_2,input_size_3), 
            ...
            fake_data_sample = torch.cat((data.view(-1,1,input_size_1,input_size_2,input_size_3), 
                                  sample_batch.view(-1,1,input_size_1,input_size_2,input_size_3)), 1)
            lossD_sample = netD(fake_data_sample, E_true=E_true)
            lossD_sample= lossD_sample.mean()
            test_lossD_sample += lossD_sample
            
            lossD_L_recon = netD_L(z)
            ...
            if epoch%args["sample_interval"] == 0 or epoch == 1:
                if i == 0:
                    n = min(data.size(0), 8)
                    comparison = torch.cat([data.view(args["batch_size"],1, input_size_1*input_size_2, input_size_3)[:n],
                                          recon_batch.view(args["batch_size"],1, input_size_1*input_size_2, input_size_3)[:n]])
...
#########################################################################
######## BIB-AE Training Energy and Angular Conditioning Reset ##########
#########################################################################
    
    
def train_BiBAE_F_linear_EandA_Reset(model, netD, netD_L, epoch, train_loader, device, args, netD_Reset,
    ...
    for batch_idx, (data, energy, theta) in enumerate(train_loader):
        data = data.to(device)
        input_size_1 = data.size(-3)
        input_size_2 = data.size(-2)
        input_size_3 = data.size(-1)
        ...
        for i in range(5):
            ...
            ######## Critic Continuous #########
            ####################################
            recon_batch, mu, logvar, z = model(data, E_true, theta_true)
            ...
            D_real = netD(real_data, E_true=E_true, theta_true=theta_true)
            D_real = D_real.mean()
            LossD = D_real*(-1.0)
            ...
            D_Reset_real = netD_Reset(real_data, E_true=E_true, theta_true=theta_true)
            ...
            D_Reset_fake = netD_Reset(fake_data, E_true=E_true, theta_true=theta_true)
        ...
        fake_data = torch.cat((data.view(-1,1,input_size_1,input_size_2,input_size_3), 
                        recon_batch.view(-1,1,input_size_1,input_size_2,input_size_3)), 1)
        lossD = netD(fake_data, E_true=E_true, theta_true=theta_true)
        lossD = lossD.mean()

        lossD_Reset = netD_Reset(fake_data, E_true=E_true, theta_true=theta_true)
        ...
        if batch_idx % args["log_interval"] == 0 or batch_idx == 0:
            image = recon_batch.view(-1, 30, 30, 60).cpu().detach().numpy()
            cmap = mpl.cm.viridis
...
def getTotE(data, xbins=30, ybins=60, layers=30):
    data = np.reshape(data,[-1, layers*xbins*ybins])
    etot_arr = np.sum(data, axis=(1))
    return etot_arr

# Occupancy
def getOcc(data, xbins=30, ybins=60, layers=30):
    data = np.reshape(data,[-1, layers*xbins*ybins])
...
##########################################################################################
######## BIB-AE Training Energy and Angular Conditioning Reset: Post Processing ##########
##########################################################################################
##### THIS MODEL ONLY CONDITIONS THE PP ON THE ENERGY SUM (NO ESUM conditioning for BIBAE base or Discriminator)!!!!
    
    
def train_BiBAE_F_linear_EandA_Reset_withA_cond_PP_ESum(model, modelP, netD, netD_L, epoch, train_loader, device, args,
    ...
    for batch_idx, (data, energy, theta) in enumerate(train_loader):
        ...
        Esum = getTotE(data_MIP_cut, xbins=30, ybins=60, layers=30)
        ...
        postprocess_batch = modelP(recon_batch.detach(), E_True=E_true, theta_True=theta_true, ESum=Esum)
        ...
        if L_D_P == 0:
            ...
            lossD_P = netD(fake_data_pp, E_true=E_true, theta_true=theta_true)
